	...
	public class Entry extends HtmlResource {
		...
		private byte[] loadType0TtfDescendantFont() throws IOException {
			...
			try {
				...
				log.warn("Error loading type 0 with ttf descendant font '{}' Message: {} {}", fontName, ex.getMessage(),
						ex.getClass());
		...
		private byte[] loadOtherTypeFont(PDStream fontFile) throws IOException {
			...
			try {
				...
				log.error("Issue converting Bare CFF font or the font type is not supported by Pdf2Dom, "
		...
		private byte[] tryNormalizeFVFont(FVFont font) {
			try {
				...
				log.warn("Error normalizing font '{}' Message: {} {}", fontName, ex.getMessage(), ex.getClass());
			}
