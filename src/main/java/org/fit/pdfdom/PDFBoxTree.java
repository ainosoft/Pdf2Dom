/**
 * PDFBoxTree.java (c) Radek Burget, 2011 Pdf2Dom is free software: you can redistribute it and/or modify it under the
 * terms of the GNU Lesser General Public License as published by the Free Software Foundation, either version 3 of the
 * License, or (at your option) any later version. Pdf2Dom is distributed in the hope that it will be useful, but
 ...
 * Public License along with CSSBox. If not, see <http://www.gnu.org/licenses/>. Created on 27.9.2011, 16:56:55 by
 * burgetr
 */
...
/**
 * A generic tree of boxes created from a PDF file. It processes the PDF document and calls the appropriate abstract
 * methods in order to render a page, text box, etc. The particular implementations are expected to implement these
 * actions in order to build the resulting document tree.
 *
 * @author burgetr
 */
public abstract class PDFBoxTree extends PDFTextStripper {
	...
	protected static String[]		cssFontFamily		= { "Times New Roman", "Times", "Garamond", "Helvetica",
			"Arial Narrow", "Arial", "Verdana", "Courier New", "MS Sans Serif" };
	 ...
	 * 
	 * @param disableGraphics
	 *            when set to <code>true</code> the graphics is ignored in the source file.
	 */
	 ...
	 * 
	 * @param disableImages
	 *            when set to <code>true</code> the images are ignored in the source file.
	 */
	 ...
	 * 
	 * @param disableImageData
	 *            when set to <code>true</code> the image data is not copied to the document tree. The eventual
	 *            <code>img</code> elements will have an empty <code>src</code> attribute.
	 */
	...
	/**
	 * Creates a new text box in the current page. The style and position of the text are contained in the
	 * {@link PDFBoxTree#curstyle} property.
	 * 
	 * @param data
	 *            The text contents.
	 */
	 ...
	 * @param stroke
	 *            should there be a stroke around?
	 * @param fill
	 ...
	 * 
	 * @param type
	 *            the image type: <code>"png"</code> or <code>"jpeg"</code>
	 * @param x
	 *            the X coordinate of the image
	 * @param y
	 *            the Y coordinate of the image
	 * @param width
	 *            the width coordinate of the image
	 * @param height
	 *            the height coordinate of the image
	 * @param data
	 *            the image data depending on the specified type
	 * @return
	 */
	...
	protected void updateFontTable() {
		...
		if (resources != null) {
			try {
				...
				log.error("Error processing font resources: " + "Exception: {} {}", e.getMessage(), e.getClass());
			}
		}
	}
		...
		for (COSName key : resources.getFontNames()) {
			...
			if (font instanceof PDTrueTypeFont) {
				...
				if (descendantFont instanceof PDCIDFontType2)
					table.addEntry(font);
				else if (descendantFont instanceof PDCIDFontType0)
					table.addEntry(font);
				else
	 ...
	 * 
	 * @param value
	 *            the PDF value of the Integer or Fload type
	 ...
	 * 
	 * @param value
	 *            the PDF value of the Integer or Float type
	 * @return the corresponging numeric value
	 */
	 ...
	 * 
	 * @param value
	 *            the PDF value of the String, Integer or Float type
